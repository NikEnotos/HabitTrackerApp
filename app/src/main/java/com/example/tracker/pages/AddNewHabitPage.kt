...
fun AddNewHabitPage(modifier: Modifier = Modifier) {
    ...
    var habitTitle by rememberSaveable(stateSaver = TextFieldValue.Saver) {
        mutableStateOf(TextFieldValue(""))
    }
    var habitDescription by rememberSaveable(stateSaver = TextFieldValue.Saver) {
        mutableStateOf(TextFieldValue(""))
    }
    ...
    Box(
        ...
        Column(
            ...
            Text(
                text = "Pick days when you should keep up this habit",
                fontSize = 15.sp,
                modifier = Modifier.align(Alignment.Start),
                color = MaterialTheme.colorScheme.onSecondaryContainer
            )
            ...
            Button(
                onClick = {
                    if (habitTitle.text.isBlank()) {
                        Toast.makeText(context, "Habit Title is required!", Toast.LENGTH_SHORT)
                            .show()
                        return@Button
                    }
                    if (selectedDays == List(7) { false }) {
                        Toast.makeText(
                            context,
                            "You have to pick at least one day",
                            Toast.LENGTH_SHORT
                        ).show()
                        return@Button
                    }
